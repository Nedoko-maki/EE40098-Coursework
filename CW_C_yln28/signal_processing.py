...
import numpy as np
from scipy.signal import find_peaks, savgol_filter, filtfilt, medfilt
...
def find_boundaries(peak_index, peaks, data, plot=False):
...
def search_peaks(peaks, data, plot=False):
    ret = []
    for i in range(len(peaks)):
        # print(peak)
        begin, end = find_boundaries(i, peaks, data, plot)
...
def pad_data(indexes, data, shape=(150,), training_data=False):
    ...
    for begin, end, *classification in indexes:
        ...
        output[insert_idx : insert_idx + len(peak)] = peak
...
def prepare_data(input_data, params):
    ...
    if "do_bandpass" in filter_options: 
        freqs, Pxx_mean, band_energy, band_db = estimate_band_from_psd(input_data, fs=25e3, plot=False)
    ...
    indexes = search_peaks(peaks, filtered, plot=False)  
    isolated_peaks = np.array(pad_data(indexes, filtered, shape=(150,)))
...
# def test_noise_reduction(data, title=""):

#     rescaled = data / np.max(data)   # scale down so the range goes from -1 to 1.
#     filtdata = filter_savgol(rescaled)

#     # savgol worked fairly well until D4-6

#     peaks, _ = find_peaks(filtdata, height=0.1, width=5, distance=10)
#     plot_data(filtdata, peaks, title=title)
